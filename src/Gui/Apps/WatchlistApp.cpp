#include "WatchlistApp.hpp"

#include "src/Gui/DeviceMonitorState.hpp"
#include "src/Gui/Workspace/DetachableApp.hpp"
...
void ShowEditor(DeviceMonitorState& state)
{
    ...
    bool const adding = editor->mode == DeviceEditorMode::Add;
    char const* title = adding ? "Add device" : "Edit device";
...
void ShowDeviceTable(DeviceMonitorState& state)
{
    ...
    for (auto const& device : state.Devices())
    {
...
void ShowHistory(char const* title, std::vector<std::string> const& lines)
{
...
void ShowMqttConsole(AppState& appState)
{
    ...
    ImGui::Combo(
        "Type", &console.selectedMessageType, messageTypeLabels.data(), static_cast<int>(messageTypeLabels.size()));
...
void ShowWatchlistAppContent(DetachableAppContext const& context)
{
    auto& state = context.deviceMonitorState;

    ImGui::TextUnformatted("Device monitor");
    ImGui::TextDisabled("Manage persisted devices and send requests through the runtime-owned MQTT client.");
    ImGui::Spacing();
    ...
    if (context.mqttState != nullptr &&
        ImGui::CollapsingHeader("MQTT console", ImGuiTreeNodeFlags_DefaultOpen))
    {
        ShowMqttConsole(*context.mqttState);
    }
